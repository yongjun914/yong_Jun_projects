...
- **Component flow is one-directional**: `App` passes `onAdd`/`onDelete` callbacks down; `src/components/IdeaForm.jsx` calls `onAdd` with validated, trimmed input; `src/components/IdeaList.jsx` renders `IdeaCard` per idea and forwards `onDelete`. No component other than `App` touches `localStorage` or holds idea state.
